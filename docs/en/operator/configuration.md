...
YAML contains non-secret defaults. Secrets — env only.

## Configuration files

| File | Service | Key sections |
|------|---------|--------------|
...
Env override for sync service URL:
...
## LLM and RAG (phase 3 + Search & RAG)

On `backend-server`:

```bash
# Chat (books, translation, RAG answers)
LLM_ENABLED=true
LLM_API_URL=http://192.168.0.64:11434/v1   # Ollama
LLM_MODEL=llama3.2:latest
LLM_API_KEY=                              # empty for local Ollama

# Embeddings (hybrid RAG retrieval)
...
```bash
MIGRATIONS_DIR=/app/migrations   # default in server Docker image
```
...
Runtime config: `/config.json` (generated by Helm ConfigMap):
...
When `frontend.proxy.enabled: true`, nginx proxies the API.
...
Settings from **System settings** are stored in PostgreSQL (`system_settings`) and override defaults for:
