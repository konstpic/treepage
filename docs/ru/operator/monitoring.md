...
Все Go-сервисы и frontend nginx expose:
...
Настроены в Helm values (enabled by default):
...
Endpoint: `/metrics` на каждом Go-сервисе.

Пример ServiceMonitor (bring your own):
...
| Сервис | Уровень (default) | Настройка |
|--------|------------------|-----------|
| auth | info | Helm: `auth.logging.level` |
| server | info | Helm: `server.logging.level` |
| sync | info | Helm: `sync.logging.level` |

UI override: **Настройки системы** → **Платформа** → **Уровень логирования**

### Audit log

При `enableAuditLog: true` (Helm: `server.security.enableAuditLog`):

- Admin actions записываются в таблицу `audit_log`
- Поля: user_id, action, resource, timestamp, details
...
Рекомендуемые limits (из Helm defaults):
