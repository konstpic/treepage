...
## Провайдер из CONFIG_PATH (карточка «Из config»)

Если OIDC настроен в YAML/env `backend-auth` (`CONFIG_PATH`), провайдер **синхронизируется в БД** при старте auth-сервиса и отображается в списке как **Authentik (config)** (или аналогично).

| Свойство | Поведение |
|----------|-----------|
| Источник | `CONFIG_PATH` / Helm `auth.oidc.*` — в UI не редактируется |
| Вход | Обрабатывает `backend-auth` |
| Админка | Просмотр issuer, client ID, redirect URL; справка по claims |
| Изменения | Правка YAML/env и **перезапуск backend-auth** |

Через **Добавить провайдера** можно вести каталог дополнительных IdP; активный провайдер входа задаётся конфигом auth-сервиса.
